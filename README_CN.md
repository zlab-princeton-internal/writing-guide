...
**2\. 即使已有完整初稿，根据以往学生经验，达到所有要求至少需要 3 天的全职工作量。请认真逐条对照，尽早开始。**
...
**AP.1** **对 appendix 采用与正文相同的写作和排版标准。** Appendix 不是垃圾堆。本指南中的所有规则——排版、图表、格式、段落结构——同样适用。


**AP.2** **如果 appendix 较长（>5 页），在开头添加目录。** 使用 `\minitoc` 或手动列表配合 `\hyperref` 链接到各 appendix section，帮助读者直接跳转。


**AP.3** **不要出现一页只有一个小图、上下大片空白的情况。** 如果图不能填满页面，要么与其他图合并，要么在周围添加更多文字，或者调整大小填充空间。


**AP.4** **不要在图周围有过多空白。** 一页上两个图之间有大间距看起来很随意。图应合理调整大小和位置以自然填充页面。


**AP.5** **段落格式要统一。** 有些段落在加粗标题后另起一行，有些接在同一行。选一种风格，在整个 appendix 中保持一致。


**AP.6** **Appendix 的段落不能全是 1-3 行。** 一连串 1-2 句的段落读起来像项目符号列表而非正文。将相关内容合并为实质性段落。Appendix 的段落可以比正文短一些，但仍应是真正的段落。


**AP.7** **考虑在 appendix 的 section 边界使用 `\newpage` 来隔离排版影响。** Appendix 图表密集，移动一个图经常导致后续页面排版连锁反应。在 section 边界插入 `\newpage` 可以防止这种连锁反应。但如果 `\newpage` 会导致前一页超过一半是空白，就不要用——那比连锁反应更难看。
...
**F.9** **Y 轴刻度值必须是整数。** 不要让 matplotlib 根据数据范围自动生成刻度值——像 30.3、71.1、96.9 这样的值看起来粗糙且不专业。手动设置为整数刻度（如 30、50、70）。
...
**F.10** **绝不允许上一节的单独一行文字溢出到下一页。** 如果段落最后一行出现在新页面顶部（"widow"），通过改写把它收回来或把更多文字推过去。Section 结尾也是如此——页面顶部的孤行看起来很不用心。


**F.11** **避免在页面顶部放置窄/稀疏的图，两侧留有大片空白。** 如果图不能填满列宽或页面宽度，会留下尴尬的空白——在页面顶部尤其明显。要么使用 `wrapfigure` 将图嵌入文字旁，要么如果图必须独占一行，就放在页面中间，上下都有文字——不要放在顶部或底部。

> **反面示例** — 页面顶部一个窄图，两侧大片空白。
>
